...
- **DNS resolution**: containers share the host's network namespace. By default, `systemd-resolved` is masked in the overlayfs upper layer at creation time so the host's resolver handles DNS. The `default_create_masked_services` config key (default: `"systemd-resolved.service"`) controls which services are masked; the `--masked-services` CLI flag overrides per container. When using `--network-zone` with config defaults, `systemd-resolved.service` is automatically removed from the mask list so LLMNR/mDNS enables inter-container name resolution. For zone containers where resolved is not masked: `/etc/resolv.conf` is symlinked to the resolved stub (`../run/systemd/resolve/stub-resolv.conf`), an LLMNR/mDNS drop-in (`resolved.conf.d/zone-llmnr.conf`) is written, and any stale `systemd-resolved.service -> /dev/null` masks in the rootfs lower layer are actively overridden in the upper layer. For non-zone containers, a placeholder `/etc/resolv.conf` regular file is written so `systemd-nspawn --resolv-conf=auto` can populate it at boot. NixOS/Nix rootfs always skip masking. When `--network-veth`, `--network-zone`, or `--network-bridge` is used, `systemd-networkd` is auto-enabled in the overlayfs upper layer so the container-side interface (`host0`) gets an IP via DHCP.
...
| `sdme fs export` | Export a rootfs or container to a directory, tarball, or raw disk image (`--filesystem ext4\|btrfs`). `--timezone` sets the timezone in the exported rootfs |
