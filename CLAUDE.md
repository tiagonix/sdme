...
| `sdme exec` | Run a one-off command in a running container (`machinectl shell`). `--oci [APP]` enters the OCI app's PID, IPC, and mount namespaces via `nsenter` (discovers the app's host PID from its cgroup, then runs `nsenter -t <pid> --pid --ipc --mount --fork`; optional app name for multi-container kube pods) |
...
- `dist/completions/` — shell completions (generated by `make dist/completions`, gitignored)
- `dist/apparmor/` — AppArmor profile (generated in CI, gitignored)
